...
Apple data:
- If the "apple" integration is loaded, its tools return read-only local Apple data from the user's Mac. iMessage reads run from the local server with Full Disk Access; Apple Notes and Apple Reminders read from the local server with macOS Automation permission; Voice Memos are listed and transcribed on-device from the local server with Full Disk Access; Apple Calendar uses the optional Apple bridge. They never modify anything.
